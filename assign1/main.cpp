...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  std::string line;
  std::ifstream csvStream(filename);
  std::getline(csvStream, line);
  Course one_cour;
  while (std::getline(csvStream, line)) {
    std::vector<std::string> tokens = split(line, ',');
    one_cour.title = tokens[0];
    one_cour.number_of_units = tokens[1];
    one_cour.quarter = tokens[2];
    courses.emplace_back(one_cour);
  }
  csvStream.close();
...
void write_courses_offered(std::vector<Course> &all_courses) {
  std::ofstream offeredCsv(COURSES_OFFERED_PATH);
  offeredCsv << "Title,Number of Units,Quarter" << std::endl;
  std::vector<Course> delCour;
  for (auto const &course : all_courses) {
    if (course.quarter == "null")
      continue;
    offeredCsv << course.title << "," << course.number_of_units << ","
               << course.quarter << std::endl;
    delCour.emplace_back(course);
  }
  offeredCsv.close();

  for (auto const &course : delCour) {
...
void write_courses_not_offered(std::vector<Course> &unlisted_courses) {
  std::ofstream offeredCsv(COURSES_NOT_OFFERED_PATH);
  offeredCsv << "Title,Number of Units,Quarter" << std::endl;
  for (auto const &course : unlisted_courses) {
    offeredCsv << course.title << "," << course.number_of_units << ","
               << course.quarter << std::endl;
  }
  offeredCsv.close();
